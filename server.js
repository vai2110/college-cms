...
function classifyPath(filePath) {
  ...
  if (
    base === 'index' ||
    ['college', 'colleges', 'exams', 'content-audit'].includes(base)
  ) {
    return null;
  }
  ...
  if (base.endsWith('-placements')) {
    return {
      collegeId: base.replace(/-placements$/, ''),
      pageId: 'placements',
      name: 'Placements',
      type: 'placement'
    };
  ...
  if (base.endsWith('-admission')) {
    return {
      collegeId: base.replace(/-admission$/, ''),
      pageId: 'admission',
      name: 'Admission',
      type: 'admission'
    };
  ...
  try {
    ...
    if (
      ...
      return res.status(503).json({
        error: 'CMS login has not been configured on the server yet.'
      });
    }
...
app.post('/api/pages', (req, res) => {
  ...
  const college = data.colleges.find(
    item => item.id === req.body.collegeId
  );
...
/* =========================================================
   LOAD PAGE
   Load a saved draft first. Otherwise load the live file.
========================================================= */
...
/* =========================================================
   SAVE DRAFT
   Saving only changes the CMS draft.
...
/* =========================================================
   PUBLISH PAGE
   Publishes the exact saved draft to GitHub.
========================================================= */
...
app.post('/api/page/:id/publish', async (req, res) => {
  try {
    ...
    /*
      Verify that the exact HTML sent by the CMS
      is now available in the GitHub repository.
    */

    const verified =
      await getWebsiteFile(targetPath);

    if (verified.html !== html) {
      throw new Error(
...
/* =========================================================
   PUBLISH HISTORY
   Reads CMS publish commits from the website repository.
========================================================= */
  ...
  try {
    ...
    const history = (
      ...
      .map(commit => {
        ...
        if (!firstLine.startsWith('CMS publish:')) {
          return null;
        }
...
const syncWebsitePages = async (req, res) => {
  try {
...
app.post('/api/sync', syncWebsitePages);
